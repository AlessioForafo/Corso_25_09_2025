...
public class DataLoggerExporter : BaseNetLogic
{
    ...
    public void Export()
    {
        try
        {
            ...
            LogicObject.GetVariable("Result").Value = "The Data logger " + tableObject.BrowseName + " has been succesfully exported to " + csvPath;     
        }
        catch (Exception ex)
        {
            Log.Error("DataLoggerExporter", "Unable to export data logger: " + ex.Message);
            LogicObject.GetVariable("Result").Value = "Unable to export data logger: " + ex.Message;
        }
